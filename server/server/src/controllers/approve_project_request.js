...
async function approve_project_request(req, res) {
  const { requestId } = req.params;
  const managerUsername = req.user.username; // guaranteed by auth middleware
  ...
  try {
    ...
    const usersToInvalidate = [joinRequest.requestuser, managerUsername];
    for (const username of usersToInvalidate) {
      const pattern = `cache:${username}:*`;
      try {
