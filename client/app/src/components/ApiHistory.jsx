...
function ApiHistory({ isApiHistoryOpen, projectId }) {
  const { theme } = useTheme();
  const isWhiteTheme = theme === "white";
  const { user } = useAuth();
  const { loadVersion } = useApiVersion();
  ...
  const handleVersionClick = async (endpoint, version) => {
    ...
    const baseurlpath = endpoint.baseUrlPath;
    if (!baseurlpath) return;
    await loadVersion(projectId, username, baseurlpath, version);
  };
  ...
  const buttonClass = isWhiteTheme
    ...
    : "text-zinc-500 hover:text-gray-200";

  return (
    <aside
      className={`flex shrink-0 overflow-hidden z-10 ${
        isWhiteTheme ? "bg-white border-l border-gray-200" : "bg-[#2b2d31] border-l border-zinc-700/50"
      }`}
    >
      <div
        className={`flex flex-col overflow-hidden transition-all duration-300 ${
          isApiHistoryOpen ? "w-40 border-r" : "w-0 border-r-0"
        } ${isWhiteTheme ? "border-gray-200" : "border-zinc-700/50"}`}
      >
        <div
          className={`text-xs font-medium border-b p-2 flex justify-center shrink-0 ${
            isWhiteTheme
        ...
        <div className="p-3 text-xs flex flex-col gap-3 flex-1 overflow-y-auto overflow-x-auto">
          ...
          {!loading && !error && historyData.length === 0 && (
            <div className="text-center text-gray-400">No API history found.</div>
          )}
          {!loading &&
            !error &&
            historyData.map((endpoint, idx) => {
              ...
              return (
                <div key={idx} className={`border rounded p-3 w-full ${cardClass}`}>
                  <p className="text-blue-400 font-medium mb-1 break-all">
                  ...
                  {!isExpanded && remaining > 0 && (
                    <button
                      ...
                      className="ml-2 mt-1 text-blue-400 hover:text-blue-300 cursor-pointer"
                    >
                      +{remaining} more...
                    </button>
                  )}
