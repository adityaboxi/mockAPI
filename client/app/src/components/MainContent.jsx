...
const ASK_AI_URL = import.meta.env.VITE_API_URL_ASK_AI;
const REVERSE_AI_URL = import.meta.env.VITE_API_URL_REVERSE_AI;
const DOMAIN = import.meta.env.VITE_DOMAIN;               // Used for OpenAPI spec and UI
const OTP_TIMER = import.meta.env.VITE_OTP_TIMER;

// ✅ Use a dedicated base URL for the mock API (without protocol)
const MOCK_API_BASE_URL = import.meta.env.VITE_MOCK_API_BASE_URL || 'api.mockapi.info';
...
function MainContent() {
  ...
  // Add path param – returns true if added, false if duplicate or empty key
  const addPathParam = useCallback(() => {
    const trimmedKey = newPathKey.trim();
    if (!trimmedKey) {
      alert('Please enter a path parameter key.');
  ...
  // Add query param – returns true if added, false if duplicate or empty
  const addQueryParam = useCallback(() => {
    const trimmedKey = newQueryKey.trim();
    const trimmedValue = newQueryValue.trim();
    if (!trimmedKey || !trimmedValue) {
  ...
  // ---------- Build final URL ----------
  const buildFinalUrl = () => {
    // Use the protocol from state and the host from MOCK_API_BASE_URL
    const baseHost = MOCK_API_BASE_URL; // e.g., "api.mockapi.info"
    let finalUrl = `${protocol}://${baseHost}`;
    let path = urlPath || '';
    path = path.replace(/[^a-zA-Z0-9/:_-]/g, '');
    path = path.replace(/\/+/g, '/');
    if (path.startsWith('/')) path = path.substring(1);
  ...
  // ---------- OpenAPI JSON (updated servers to use MOCK_API_BASE_URL) ----------
  const openApiJson = useMemo(() => {
    let requestSchema = { type: "object", properties: {} };
    let responseSchema = { type: "object", properties: {} };
    try {
    ...
    return {
      openapi: "3.0.3",
      info: { title: currentProject?.name || "Dynamic API Asset Workspace", description: "Auto-compiled specification documentation.", version: "1.0.0" },
      servers: [{ url: `${protocol}://${MOCK_API_BASE_URL}` }],   // ✅ Use correct base
      paths: {
        [formattedPath]: {
          [method.toLowerCase()]: {
            summary: "Auto-updated route specification",
