...
function MainContent() {
  ...
  // ✅ New state for curl copy animation
...
const generateCurlCommand = useCallback(() => {
 ...
 const handleCopyCurl = useCallback(() => {
  ...
  // Fallback for HTTP (clipboard API unavailable)
...
// --- Import your socket instance ---
import { socket } from "../socket"; // or use useSocket() hook
...
function MainContent() {
  ...
  // ✅ NEW: Join the user room for Socket.IO AI events
  useEffect(() => {
    if (!socket || !socket.connected || !user?.username) {
      console.log('[MainContent] Skipping room join – socket not ready or no user');
      return;
    }

    const roomName = `user:${user.username}`;
    socket.emit('join_room', roomName);
    console.log(`[MainContent] ✅ Joined room: ${roomName}`);

    const onReconnect = () => {
      socket.emit('join_room', roomName);
      console.log(`[MainContent] ✅ Re-joined room: ${roomName}`);
    };
  ...
  // Helper to apply AI result (same as before)
  ...
  // Start fallback polling if no WebSocket response within 3 seconds
  const startPollFallback = useCallback((jobId) => {
    if (pollFallbackTimerRef.current) clearTimeout(pollFallbackTimerRef.current);
    pollFallbackTimerRef.current = setTimeout(() => {
      // Only poll if still loading and same jobId
      if (isAiLoading && currentJobId === jobId) {
        pollForResult(jobId);
      }
    }, 3000);
  }, [isAiLoading, currentJobId, pollForResult]);

  // ✅ CHANGED: WebSocket listeners with debug logs
  useEffect(() => {
    if (!socket) return;

    const onResponse = (data) => {
      console.log('[MainContent] 🔵 ai:response received:', data);
    ...
    const onChunk = (data) => {
      console.log('[MainContent] 🔵 ai:chunk received:', data);
      if (data.jobId === currentJobId) {
    ...
    const onError = (data) => {
      console.log('[MainContent] 🔴 ai:error received:', data);
      if (data.jobId === currentJobId) {
  ...
  // Recovery on mount: check for pending job in localStorage
  useEffect(() => {
  ...
  // New handleAskAi (enqueue + store jobId)
  const handleAskAi = async () => {
    const parsedRequestBody = safeParseJSON(requestBody);
    const parsedResponseBody = safeParseJSON(responseBody);
    const payload = {
    ...
    try {
      ...
      // If server returns result directly (e.g., cached or fallback), apply immediately
  ...
  // Status code handlers (unchanged)
  const handleStatusCodeChange = (e) => {
    const rawValue = e.target.value;
    if (rawValue === '') {
      setStatusCode('');
      ...
      <div className={`shrink-0 border-t z-20 flex flex-col relative block ${w ? "border-gray-200 bg-white" : "border-zinc-700/50 bg-[#1e1f22]"}`}>
        ...
        <div className="relative w-full h-24 shrink-0 block overflow-visible z-10">
          <textarea
            ...
            className={`w-full h-full px-3 pt-3 pb-12 resize-none outline-none text-sm block relative z-0 ${w ? "bg-white text-gray-800 placeholder-gray-400 focus:border-gray-300" : "bg-[#1e1f22] text-gray-300 placeholder-zinc-600 focus:border-zinc-700"}`}
