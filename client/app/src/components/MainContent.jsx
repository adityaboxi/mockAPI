...
function MainContent() {
  ...
  useEffect(() => {
    ...
    setLatency(currentVersionData.latency || 0);
    setRateLimit(currentVersionData.rateLimit || 0);
    setHeaders(currentVersionData.headers || []);
    setResponseHeaders(currentVersionData.responseHeaders || []);
              ...
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                <div className="relative">
                  <input
                    ...
                    className="sr-only peer"
                  />
                  <div className={`w-8 h-4 rounded-full transition-colors ${includeAIResponse ? "bg-blue-600" : w ? "bg-gray-300" : "bg-zinc-700"}`} />
                  <div className={`absolute left-0.5 top-0.5 w-3 h-3 rounded-full bg-white transition-transform ${includeAIResponse ? "translate-x-4" : ""}`} />
                </div>
              </label>
            </div>
            <div className="flex items-center gap-2">
