...
function ProjectDetailsModal({ project, isOpen, onClose, onStatusChange, onInvitationCodeUpdated }) {
  ...
  useEffect(() => {
    ...
    fetch(API_PROJECTS, { credentials: "include" })
      ...
      .catch(console.error);
  }, [isOpen, project.id, project.invitationCode]);
  ...
  const handleResetCode = async () => {
    ...
    try {
      const res = await fetch(API_RESET_INVITE, {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ project_id: project.id, projectName: project.projectname }),
      });
      if (res.ok) {
        setShowOtpSection(true);
        setTimer(120);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const res = await fetch(API_VERIFY_INVITE_OTP, {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ project_id: project.id, otp: otpCode }),
      });
      const data = await res.json();
      if (res.ok) {
        setLocalInvitationCode(data.newInvitationCode);
        onInvitationCodeUpdated(project.id, data.newInvitationCode);
        setShowOtpSection(false);
        setOtpCode("");
        onClose();
      } else {
  ...
  const handleStatusChange = async (newStatus, target) => {
    ...
    setStatusUpdating(true);
    try {
      const res = await fetch(`${API_UPDATE_PROJECT_STATUS}/${project.id}/status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ isActive: newStatus }),
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      const response = await fetch(API_DELETE_PROJECT, {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ invitationCode: localInvitationCode }),
      });
