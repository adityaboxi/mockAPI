...
function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // authenticateUser now throws on failure and returns user data on success
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center font-sans selection:bg-blue-500/30 ${
        isWhiteTheme ? "bg-gray-100" : "bg-[#1e1e24]"
      }`}
    >
      <div
        className={`p-8 rounded shadow-lg border w-full max-w-sm ${
          isWhiteTheme ? "bg-white border-gray-200" : "bg-[#2b2d31] border-zinc-700/50"
        }`}
      >
        <div className="mb-4">
          <h1
            className={`text-xs font-medium mb-1 ${
              isWhiteTheme ? "text-gray-500" : "text-gray-400"
            }`}
          >
            username
          </h1>
        ...
        <div className="mb-6">
          <span
            className={`text-xs inline-block mr-2 ${
              isWhiteTheme ? "text-gray-500" : "text-gray-500"
            }`}
          >
            not logged in?
          </span>
