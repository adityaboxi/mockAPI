...
function Subscribe() {
  ...
  const mainWrapperBg = isWhite ? "bg-white text-gray-800" : "bg-zinc-950 text-zinc-300";
  const cardBg = isWhite ? "bg-gray-50 border border-gray-200" : "bg-zinc-900 border border-zinc-800";
  const headerTxt = isWhite ? "text-gray-900" : "text-white";
  const mutedTxt = isWhite ? "text-gray-500" : "text-zinc-400";
  const dividerLine = isWhite ? "border-gray-200" : "border-zinc-800";
          ...
          <ul className="space-y-2">
            {features.map((feature, idx) => (
              <li key={idx} className="flex items-start gap-2.5 text-xs">
                ...
                <span className={isWhite ? "text-gray-600" : "text-zinc-300"}>{feature}</span>
              </li>
            ))}
          </ul>
        </div>
