...
// 🔒 SECURITY: Trust the first proxy (OpenResty) to get real client IP
app.set('trust proxy', true);

// ✅ Compression – reduces bandwidth (already enabled)
...
redis.on('error', (err) => console.warn('[Redis] Connection error (will retry):', err.message));
...
// ---------- Radix Tree Router (in‑memory) ----------
const router = Router({
  ignoreTrailingSlash: true,
  maxParamLength: 500,
});

// In‑memory route storage – preserved during pause/unpause
...
function registerRoute(definition) {
  ...
  if (registeredKeys.has(key)) {
    ...
    if (existing && JSON.stringify(existing) === JSON.stringify(definition)) {
      log(`[Router] Route identical, skipping: ${key}`);
      return true;
    }
    log(`[Router] Route changed, updating: ${key}`);
  }

  try {
    router.on(definition.method, fullPath, () => {}, definition);
  } catch (err) {
    console.error(`[Router] Failed to register route ${key}:`, err.message);
    return false;
  }

  routeDefinitions.set(key, definition);
  registeredKeys.add(key);
  console.log(`[Router] ✅ Route added/updated: ${key} (total: ${registeredKeys.size})`);
  return true;
}
...
function unregisterRoute(method, fullPath) {
  ...
  try {
    ...
    console.warn(`[Router] Failed to remove route ${key}:`, err.message);
    return false;
  }
  registeredKeys.delete(key);
  routeDefinitions.delete(key);
  console.log(`[Router] ❌ Removed route: ${key} (remaining: ${registeredKeys.size})`);
  return true;
}
...
function trimFakerCache() {
  while (fakerCache.size >= MAX_FAKER_CACHE) {
    const firstKey = fakerCache.keys().next().value;
    if (firstKey !== undefined) fakerCache.delete(firstKey);
    else break;
  }
}
...
function getFakerValue(path) {
  ...
  try {
    ...
    console.error(`[Faker Resolver Error] Failed on path "${path}":`, err.message);
    return null;
  }
}
...
app.delete('/internal/apis', (req, res) => {
  ...
  if (!registeredKeys.has(routeKey)) {
    log(`[Router] Route already deleted, skipping: ${routeKey}`);
...
app.post('/internal/sync', (req, res) => {
  // No‑op – we only keep routes in memory
  // This endpoint exists to keep the worker‑server happy.
...
app.all('*', async (req, res) => {
  ...
  // Latency offloaded to Nginx (only header)
  if (definition.latency && definition.latency > 0) {
    res.set('X-Latency', String(Math.min(Number(definition.latency), 30000)));
  }
...
app.use((err, req, res, next) => {
  console.error('[project-container] Unhandled error:', err.stack);
